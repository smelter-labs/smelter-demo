...
const AVAILABLE_SHADERS = [
    ...
    },
    {
        ...
        description: 'Masks the image to a circle with a given diameter and adds an outline with configurable width and hue.',
    ...
    },
    {
        ...
        description: 'Applies a vertical sine-wave distortion along X with configurable amplitude, wavelength, speed and phase.',
    ...
    },
    {
        ...
        isVisible: true,
        name: 'Star Streaks',
        description: 'Adds animated, parameterized white streaks (warp-star effect) over the input, simulating fast motion through space.',
    ...
    },
    {
        ...
        isVisible: true,
        name: 'Soft Shadow',
        description: 'Adds a soft, slightly animated shadow behind the input. Color, opacity, offset and blur are configurable.',
...
class ShadersController {
    get shaders() {
        const baseIconsDir = path_1.default.resolve(__dirname, '../../shaders/icons');
        return AVAILABLE_SHADERS.filter(shader => shader.isActive).map(shader => {
            const iconPath = path_1.default.join(baseIconsDir, `${shader.id}.svg`);
            let iconSvg = '';
            try {
                iconSvg = fs_1.default.readFileSync(iconPath, { encoding: 'utf-8' });
            }
            catch {
                iconSvg =
...
exports.default = shadersController;
