...
export function KickAddInputForm({
    ...
    const [kickSuggestions, setKickSuggestions] = useState<
        Array<{ streamId: string; displayName: string; title: string; category: string }>
    >([]);
        ...
        <GenericAddInputForm
                ...
                kickSuggestions
                    .filter((suggestion) => {
                        for (const input of inputs) {
                            if (
                                (input as any).kickChannelId === suggestion.streamId
                            ) {
                                return false;
                            }
                        }
            ...
            renderSuggestion={(suggestion, idx, highlighted) => (
                <>
                    ...
                    <span className='ml-2 text-white-60 block'>[Kick.com] {suggestion.category}</span>
                    <br />
