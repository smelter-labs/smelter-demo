  ...
  const filteredSuggestions = suggestions.twitch
    ...
    .filter((suggestion) => {
      ...
      return (
        ...
        suggestion.title.toLowerCase().includes(currentSuggestion.toLowerCase())
      );
    });
...
export function AddMP4InputForm({
  ...
  // Fetch mp4 suggestions on mount
  useEffect(() => {
    // getMP4Suggestions returns a Promise<MP4Suggestions>
    getMP4Suggestions().then(setMp4Suggestions);
  }, []);

  // Do not filter suggestions
  const filteredSuggestions = mp4Suggestions.mp4s;
  ...
  function handleInputKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (
        ...
        e.preventDefault();
        setCurrentSuggestion(filteredSuggestions[highlightedIndex]);
        setShowSuggestions(false);
        setHighlightedIndex(-1);
      }
    } else if (e.key === 'Escape') {
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        e.preventDefault();
        setLoading(true);
        const mp4Url = currentSuggestion.trim();
        if (!mp4Url) {
        ...
        try {
          await addMP4Input(roomId, mp4Url);
          await refreshState();
          setCurrentSuggestion('');
        } catch (err) {
          toast.error(`Failed to add "${mp4Url}" MP4 input.`);
        } finally {
          setLoading(false);
        }
      }}>
        ...
        <AnimatePresence>
          {showSuggestions && filteredSuggestions.length > 0 && (
            <motion.div
              ...
              {filteredSuggestions.map((mp4Url, idx) => (
                <button
                  type='button'
                  key={mp4Url}
                  className={`w-full text-left px-3 py-2 hover:bg-purple-80 hover:text-white-100 focus:bg-purple-100 focus:text-white-100 transition-colors
      ...
      <Button
        ...
        type='submit'
        disabled={loading}>
        {loading ? <LoadingSpinner size='sm' variant='spinner' /> : 'Add MP4'}
      </Button>
    </form>
  );
}
